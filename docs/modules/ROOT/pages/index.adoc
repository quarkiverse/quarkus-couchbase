...
# Optional, the name of an existing bucket to inject as a `Bucket` bean. If using DevServices, this bucket is also provisioned in the container.
quarkus.couchbase.bucket-name=travel-sample
----

You can now `@Inject` a Couchbase `Cluster` and `Bucket` into your project:
